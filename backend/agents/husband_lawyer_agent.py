...
- 남편 측에 유리한 주장을 구성하되, 불리한 약점도 숨기지 않는다.
- 확정적 법률 결론을 피하고, "쟁점이 될 가능성" 중심으로 작성한다.
- JSON으로만 출력한다.

출력 스키마:
{
  "agent_name": "HusbandLawyerAgent",
  "summary": "string",
  "claims": [
    {
      "claim_id": "H_CLAIM_001",
...
async def run(case_info: dict, evidence_items: list, upstage_service: UpstageService | None = None) -> dict:
    ...
    result = await upstage.call_solar_json(
        ...
        user_payload={
            ...
            "task": "남편 측에 유리한 evidence-grounded claim을 만들고, 각 claim마다 가능한 반박도 작성하라.",
        },
        temperature=0.25,
    )
    if result.get("fallback"):
        return {
            "agent_name": "HusbandLawyerAgent",
            "summary": "Solar 호출이 설정되지 않아 남편 측 주장을 생성하지 못했습니다.",
            "claims": [],
            "weaknesses": ["UPSTAGE_API_KEY 설정이 필요합니다."],
            "missing_evidence": [],
            "error": result.get("error"),
        }
    result.setdefault("agent_name", "HusbandLawyerAgent")
...
DISCUSSION_PROMPT = """
너는 남편 측 AI 변호사 에이전트다.
역할:
- 이미 생성된 남편 측 주장과 아내 측 주장을 비교한다.
- 아내 측 주장 중 반박할 부분, 일부 인정할 부분, 추가 증거가 필요한 부분을 구분한다.
- evidence_items에 있는 내용만 사용한다.
...
{
  "agent_name": "HusbandLawyerAgent",
  "turn_type": "response_to_wife",
  "messages": [
    {
      "message_id": "H_TURN_001",
      "speaker": "남편 측 변호사",
      "content": "string",
...
async def respond_to_opponent(
    ...
    result = await upstage.call_solar_json(
        ...
        user_payload={
            ...
            "task": "아내 측 주장을 검토하고, 남편 측 입장에서 evidence-grounded response turn을 작성하라.",
        },
        temperature=0.25,
    )
    if result.get("fallback"):
        return {
            ...
            "turn_type": "response_to_wife",
            "messages": [],
            "summary": "Solar 호출이 설정되지 않아 남편 측 응답 turn을 생성하지 못했습니다.",
            "remaining_disputes": [],
            "missing_evidence": [],
            "error": result.get("error"),
        }
    result.setdefault("agent_name", "HusbandLawyerAgent")
    result.setdefault("turn_type", "response_to_wife")
